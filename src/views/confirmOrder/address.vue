<template>
    <div class="address height100" v-loading="addressLoading">
        ...
        <div class="address-content marginT10">
            <ul class="address-list">
                <li v-for="(item, index) in addressList" :key="index" @click="choiceAddress(item, index)" :class="{active: choiceIndex == index}">
...
<script>
    ...
    const CHOICE_INDEX = 'CHOICE_INDEX';
    ...
    export default {
        ...
        methods: {
            ...
            addAddress() {
                this.$router.push({name: 'caddAddress'});
            },
            getAddressList() {
            ...
            choiceAddress(item, index) {
                this.$store.commit(CHOICE_INDEX, index);
                this.$store.commit(DEFAULT_ADDRESS, item);
                this.$router.back(-1);
            }
        },
        created() {
        ...
        computed: {
            ...
            choiceIndex() {
                return this.$store.getters.choiceIndex
            }
        },
        watch: {
            isAddressChange() {
